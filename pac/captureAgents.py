...
class RandomAgent( Agent ):
  ...
  def getAction(self, state: 'GameState') -> None:
...
class CaptureAgent(Agent):
  ...
  def __init__(self, index: int, timeForComputing=.1):
    """
    ...
    self.agentsOnTeam: list[Self] = None

    # Maze distance calculator
    self.distancer: distanceCalculator.Distancer = None

    # A history of observations
    self.observationHistory = []

    # Time to spend each turn on computing maze distances
    self.timeForComputing: float = timeForComputing

    # Access to the graphics
    self.display: 'Graphics' = None
  ...
  def registerTeam(self, agentsOnTeam: list[Self]) -> None:
    """
  ...
  def observationFunction(self, gameState: 'GameState') -> 'GameState':
    " Changing this won't affect pacclient.py, but will affect capture.py "
    return gameState.makeObservation(self.index)

  def debugDraw(self, cells: list, color: list[float], clear=False) -> None:
  ...
  def getFood(self, gameState: 'GameState'):
    """
