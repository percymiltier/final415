...
class Action(enum.Enum):
    pass
@enum.unique
class Directions(enum.StrEnum, Action):
    NORTH = 'North'
    SOUTH = 'South'
    EAST = 'East'
    WEST = 'West'
    ...
    RIGHT = enum.nonmember({(y, x) for x, y in LEFT.items()})

    REVERSE = enum.nonmember({
        NORTH: SOUTH,
        SOUTH: NORTH,
        EAST: WEST,
        WEST: EAST,
...
class Configuration:
    ...
    def __init__(self, pos: tuple[int, int], direction: Directions):
        self.pos = pos
    ...
    def generateSuccessor(self, vector: tuple[int, int]) -> Self:
        """
...
class Grid:
    ...
    def _unpackBits(self, bits) -> None:
        """
...
class Actions:
    ...
    @staticmethod
    def reverseDirection(action: Action) -> Action:
    ...
    @staticmethod
    def vectorToDirection(vector: tuple[int, int]) -> Action:
        dx, dy = vector
        if dy > 0:
            return Directions.NORTH
        if dy < 0:
    ...
    @staticmethod
    def directionToVector(direction: Action, speed: float = 1.0) -> tuple[float, float]:
        dx, dy =  Actions._directions[direction]
        return (dx * speed, dy * speed)

    @staticmethod
    def getPossibleActions(config: Configuration, walls: list[list[bool]]) -> list[Action]:
        possible = []
        x, y = config.pos
    ...
    @staticmethod
    def getLegalNeighbors(position: tuple[int, int], walls) -> list[tuple[int, int]]:
        x,y = position
        x_int, y_int = int(x + 0.5), int(y + 0.5)
        neighbors = []
        for dir, vec in Actions._directionsAsList:
    ...
    @staticmethod
    def getSuccessor(position: tuple[int, int], action: Directions) -> tuple[int, int]:
...
class GameStateData:
    ...
    """
    def __init__(self, prevState: 'GameState | None' = None):
        """
        Generates a new data packet by copying information from its predecessor.
        """
        if prevState != None:
            self.food: Grid = prevState.food.shallowCopy()
            self.capsules: list[tuple[int, int]] = prevState.capsules[:]
        ...
        self._foodEaten: tuple[int, int] = None
        self._foodAdded: list[tuple[int, int]] = None
        self._capsuleEaten: tuple[int, int] = None
        self._agentMoved: int = None
    ...
    def copyAgentStates(self, agentStates: list[tuple[int, int]]) -> list[tuple[int, int]]:
        copiedStates = []
    ...
    def _pacStr(self, dir: Action) -> str:
    ...
    def _ghostStr(self, dir: Action) -> str:
        return 'G'
        if dir == Directions.NORTH:
            return 'M'
        if dir == Directions.SOUTH:
...
class Game:
    ...
    def __init__(self,
                 agents: list[Agent],
                 display: 'Graphics',
                 rules: 'CaptureRules | ClassicGameRules',
                 startingIndex: int = 0,
        ...
        import io
        self.agentOutput: list[io.StringIO] = [io.StringIO() for agent in agents]
    ...
    def _agentCrash( self, agentIndex: int, quiet=False) -> None:
