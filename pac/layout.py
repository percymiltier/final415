...
class Layout:
    ...
    def __init__(self, layoutText: list[str]):
        ...
        self.agentPositions: list[tuple[int, tuple[int, int]]] = []
        self.numGhosts = 0
        self.processLayoutText(layoutText)
        self.layoutText = layoutText
        self.totalFood: int = len(self.food.asList())
    ...
    def getFurthestCorner(self, pacPos: tuple[int, int]) -> tuple[int, int]:
        poses = [(1,1), (1, self.height - 2), (self.width - 2, 1), (self.width - 2, self.height - 2)]
        dist, pos = max([(util.manhattanDistance(p, pacPos), p) for p in poses])
        return pos

    def isVisibleFrom(self, ghostPos: tuple[int, int], pacPos: tuple[int, int], pacDirection) -> bool:
...
def tryToLoad(fullname: str) -> None:
